  ...
  <div class="relative bg-gray-50 px-6 pt-16 pb-20 lg:px-8 lg:pt-24 lg:pb-28">
    ...
    <div class="relative mx-auto max-w-7xl">
      <div class="text-center">
        <h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">博客文章</h2>
        <p class="mx-auto mt-3 max-w-2xl text-xl text-gray-500 sm:mt-4">阅读文章，获取更多灵感。</p>
      </div>
          ...
          <div class="flex flex-1 flex-col justify-between bg-white p-6">
            <div class="flex-1">
              <p class="text-sm font-medium text-indigo-600">
                ...
                <NuxtLink :to="{name:'blog-tag-id',params:{tag:t.ID}}" class="hover:underline" v-for="t in post.tag" :key="t.ID">{{ t.name }}</NuxtLink>
                </span>
              </p>
...
const {data} = await useAsyncData("home",async () => {
  const [listRes, dictRes] = await Promise.all([
    getCmsArticleList({page:1, pageSize:10, tag: route.params.id}),
