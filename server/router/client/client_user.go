...
func (s *ClientUserRouter) InitClientUserRouter(Router *gin.RouterGroup, PublicRouter *gin.RouterGroup, ClientPrivateRouter *gin.RouterGroup) {
	...
	clientUserRouterPrivate := ClientPrivateRouter.Group("clientUser")
	...
	{
		clientUserRouterPrivate.GET("getUserInfo", clientUserApi.GetUserInfo)     // 获取用户信息（需要登录）
		clientUserRouterPrivate.PUT("setClientUser", clientUserApi.SetClientUser) // 更新客户端用户（需要登录）
	}
}
